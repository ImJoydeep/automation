...
def update_masterlist(uri,client_id,client_esp):
    ...
    try:
        campaign_data_df = pd.read_csv(uri + client_id +'-Blueshift-Campaigns.csv')
    except:
        dt_start = datetime.now().strftime("%d/%m/%Y %I:%M:%S")
        main_module.logs(dt_start, 'Blueshift-Campaigns.csv', '0 rows', '0 rows','failed',client_id,client_esp)
        return
    ...
    try:
        ...
        dt_start = datetime.now().strftime("%d/%m/%Y %I:%M:%S")
        main_module.logs(dt_start,'Blueshift-Trigger.csv', '0 rows', '0 rows', 'failed',client_id,client_esp)
        return
    ...
    files_list = [client_id+'-Blueshift-Campaigns.csv', client_id+'-Blueshift-Trigger.csv', client_id+'-Campaign-Analytics.csv', client_id+'-Trigger-Analytics.csv', client_id+'-Trigger-Types.csv']
