...
def update_masterlist(uri,client_id,client_esp):
    ...
    try:
        campaign_data_df = pd.read_csv(uri + client_id +'-Mailchimp-Campaigns.csv')
    except:
        dt_start = datetime.now().strftime("%d/%m/%Y %I:%M:%S")
        main_module.logs(dt_start, 'Mailchimp-Campaigns.csv', '0 rows', '0 rows','failed',client_id,client_esp)
        return
    ...
    try:
        ...
        dt_start = datetime.now().strftime("%d/%m/%Y %I:%M:%S")
        main_module.logs(dt_start,'Mailchimp-Trigger.csv', '0 rows', '0 rows', 'failed',client_id,client_esp)
        return
    ...
    files_list = [client_id+'-Mailchimp-Campaigns.csv', client_id+'-Mailchimp-Trigger.csv', client_id+'-Campaign-Analytics.csv', client_id+'-Trigger-Analytics.csv', client_id+'-Trigger-Types.csv']
