...
def update_masterlist(uri,client_id,client_esp):
    ...
    try:
        campaign_data_df = pd.read_csv(uri + client_id +'-Epsilon-Campaigns.csv')
    except:
        dt_start = datetime.now().strftime("%d/%m/%Y %I:%M:%S")
        main_module.logs(dt_start, 'Epsilon-Campaigns.csv', '0 rows', '0 rows','failed',client_id,client_esp)
        return
    ...
    try:
        ...
        dt_start = datetime.now().strftime("%d/%m/%Y %I:%M:%S")
        main_module.logs(dt_start,'Epsilon-Trigger.csv', '0 rows', '0 rows', 'failed',client_id,client_esp)
        return
    ...
    files_list = [client_id+'-Epsilon-Campaigns.csv', client_id+'-Epsilon-Trigger.csv', client_id+'-Campaign-Analytics.csv', client_id+'-Trigger-Analytics.csv', client_id+'-Trigger-Types.csv']
