...
client_name = 'Sunday Lawn Care'
dbx_path = '/Client/Sunday Lawn Care/Internal Files/Reporting/'
...
def update_masterlist(uri,client_id,client_esp):
    ...
    error_result = ''
    # uri = '/home/nav93/Downloads/AlchemyWroxFiles/Sunday_Lawn_Care/2023_0604-0610/' #local
    ...
    try:
        ...
        print("Campaign-Analytics-rev",e)
    ...
    try:
        ...
        print("Campaign-Analytics-ses",e)
    ...
    if not trigger_data_df_2.empty:
        ...
        initial_revenue = round(campaign_data_df['Unique Order Completed - Server'].sum() + trigger_data_df['Order Completed - Server Value'].sum() + trigger_data_df_2['Order Completed - Server Value'].sum(), 2)    
    else:
        initial_delivered = round(campaign_data_df['Successful Deliveries'].sum() + trigger_data_df['Delivered'].sum(), 2) 
        initial_revenue = round(campaign_data_df['Unique Order Completed - Server'].sum() + trigger_data_df['Order Completed - Server Value'].sum(), 2) 
    ...
    campaign_data_df['Variant Name'] = campaign_data_df['Variant Name'].replace(' ', "-", regex=True)
    campaign_data_df['Variant Name'] = campaign_data_df['Variant Name'].replace('_', "-", regex=True)
    ...
    # main_campaign_analytics_df.to_csv(uri + 'Campaign GA.csv', index=False)
    ...
    # main_trigger_df.to_csv(uri+"maintriggerfile.csv",index = False)
    # main_trigger_analytics_df.to_csv(uri+"triggergafile.csv",index =False)
    ...
    if initial_revenue != final_revenue:
        print('test -------',initial_revenue,final_revenue)
    ...
    str_cols = ['Campaign','Mailing','Variant','Subject_Line','Segment','Type_2','Campaign_ID','ESP','Flow_Message_ID','Flow_ID','Flow_Name',]
    ...
    filter_date = dt.datetime.strptime('2020-01-21', "%Y-%m-%d")
    filter_date = filter_date.date()
    print(filter_date, type(filter_date))
    dedup = updated_masterlist.loc[(updated_masterlist['Date'] >= filter_date)].drop_duplicates(subset=['Date','Name','Subject_Line','Segment','Variant','Flow_Name',"Flow_Message_ID"], keep='last')
    updated_masterlist = updated_masterlist.loc[(updated_masterlist['Date'] < filter_date)].append(dedup)
    # Adding Old Campaign
    # updated_masterlist.to_csv(uri+"alldataafterfilter.csv",index = False)
    rev_order_df = pd.merge(rev_order_rev_df,rev_order_ses_df,on="Date",how="outer")
    # old_campaign_data = main_module.process_old_cam
    # paign(uri,client_id,updated_masterlist,rev_order_df,session_df)
    ## Amit
    print("before old camp",updated_masterlist.shape)
    old_campaign_data = process_old_campaign(uri,client_id,updated_masterlist,rev_order_df)
    print("old camp data shape",old_campaign_data.shape)
    ...
    #updated_masterlist = updated_masterlist.drop_duplicates(subset=['Date','Name','Subject_Line','Segment','Variant','Flow_Message_Name'], keep='last').sort_values('Date')
    # filter_date1 = dt.datetime.strptime('2022-12-31', "%Y-%m-%d")
    # filter_date1 = filter_date1.date()
    # deduped = updated_masterlist.loc[(updated_masterlist['Date'] >= filter_date1)].drop_duplicates(subset=['Date','Name','Subject_Line','Segment','Variant'], keep='last').sort_values('Date')
    # updated_masterlist = updated_masterlist.loc[(updated_masterlist['Date'] < filter_date1)].append(deduped)
    ...
    cols_all = ["Date","Name","Campaign","Mailing","Variant","Subject_Line","Segment","Type_0","Type_1","Type_2","Offer","Segment_Engagement","Segment_BNB","Segment_Freq","Test_Type","Sent","Delivered","Opens","Clicks","Revenue","Orders","GA_Revenue","GA_Orders","GA_Sessions","AA_Visits","AA_Revenue","AA_SFCC_Demand","AA_Orders","AA_Mobile_Visits","Units","Product_Margin","Total_Margin","Unsub","Complaints","Hard_Bounces","Soft_Bounces","Total_Bounces","ESP","Total_Opens","Total_Clicks","Campaign_ID","Flow_ID","Flow_Name","Flow_Message_ID","Creatives","Error","Original_Segment","FISCAL_YEAR"]
    ...
    dbx_rename = {"Name" :"Name","Campaign" :"Campaign","Mailing" :"Mailing","Test_Type": "Test Type","Variant" :"Variant","Offer": "Offer","Subject_Line" :"Subject Line","Segment" :"Segment","Date" :"Date","ESP" :"ESP","Type_0" :"Type 0","Type_1" :"Type 1","Type_2" :"Type 2","Sent" :"Sent","Orders" :"Orders","Revenue" :"Revenue","Opens" :"Opens","Total_Opens" :"Total Opens","Clicks" :"Clicks","Total_Clicks" :"Total Clicks","Unsub" :"Unsub","Complaints" :"Complaints","Delivered" :"Delivered","Total_Bounces" :"Total Bounce","Campaign_ID" :"Campaign ID","Flow_ID" :"Flow ID","Flow_Name" :"Flow Name","Flow_Message_ID" :"Flow Message ID","GA_Revenue" :"GA Revenue","GA_Orders" :"GA Orders","GA_Sessions" :"GA Sessions"}
    dbx_schema = ["Name","Campaign","Mailing","Test Type","Variant","Type 2","Offer","Subject Line","Segment","Date","ESP","Type 0","Type 1","Sent","Orders","Revenue","Opens","Total Opens","Clicks","Total Clicks","Unsub","Complaints","Delivered","Total Bounce","Campaign ID","Flow ID","Flow Name","Flow Message ID","GA Revenue","GA Orders","GA Sessions","Creatives"]
    dbx_masterlist = updated_masterlist
    dbx_masterlist.rename(columns=dbx_rename, inplace=True)
    dbx_masterlist['Creatives'] = ''
...
def Trigger_Transform(trigger_data_df,client_id,client_esp,trigger_name,trigger_flow_name):
    ...
    rename_col ={
        ...
        'Order Completed - Server Value': 'Revenue',
        'Unique Order Completed - Server': 'Orders',
        'Order Completed - Server': 'Orders',
