...
def campaign_ga_join(df_esp,df_ga):
    ...
    df_merge[['GA_Orders', 'GA_Sessions','GA_Revenue']] = df_merge[['GA_Orders', 'GA_Sessions','GA_Revenue']].fillna(0) 
        ...
        if(len(data) > 5):
            if("-" in data):
                ...
                if(len(newformat[0]) == 4):
                    format = "%Y-%m-%d"
                elif (len(newformat[-1]) == 4 and int(newformat[0]) < 13):
                    format = "%m-%d-%Y"
                else:
                    format = "%d-%m-%Y"
