...
def update_masterlist(uri,client_id,client_esp):
    ...
    if not trigger_data_df_2.empty:
        ...
        initial_revenue = round(campaign_data_df['Subscription Started Value'].sum() + trigger_data_df['Subscription Started Value'].sum() + trigger_data_df_2['Subscription Started Value'].sum(), 2)    
    else:
        initial_delivered = round(campaign_data_df['Successful Deliveries'].sum() + trigger_data_df['Delivered'].sum(), 2) 
        initial_revenue = round(campaign_data_df['Subscription Started Value'].sum() + trigger_data_df['Subscription Started Value'].sum(), 2) 
    ...
    if trigger_data_df_2.empty:
        ...
        main_trigger_masterlist_df_2 = campaign_ga_join(main_trigger_df_2,main_trigger_analytics_df_2)
...
def campaign_ga_join(df_esp,df_ga):
    ...
    df_merge[['GA_Order', 'GA_Sessions','GA_Revenue']] = df_merge[['GA_Order', 'GA_Sessions','GA_Revenue']].apply(pd.to_numeric) 
