...
def update_masterlist(uri,client_id,client_esp):
    ...
    if not trigger_data_df_2.empty:
        ...
        initial_revenue = round(campaign_data_df['Ordered Product Value'].sum() + trigger_data_df['Ordered Product Value'].sum() + trigger_data_df_2['Ordered Product Value'].sum(), 2)    
    else:
