...
client_name = 'GAIAM'
dbx_path = '/Client/Gaiam/Internal Files/Reporting/Tableau/Gaiam/'
...
def update_masterlist(uri,client_id,client_esp):
    ...
    for file in list_file:
        ...
        if 'MessageActivity' in file_name:
            ...
            try:
                df = pd.read_csv(uri + file_name)
                df['Publish Date (UTC-07)'] = trigger_date
    ...
    #print(main_trigger_df[['Delivered']].to_string())
    ...
    int_cols = ['Sent','Delivered','Opens','Clicks','Unsub','Complaints','Total_Bounces','Orders','Revenue']
    ...
    masterlist_bigquery['Date'] = pd.to_datetime(masterlist_bigquery['Date']).dt.strftime('%Y-%m-%d')
    ...
    dedup = updated_masterlist.loc[(updated_masterlist['Date'] >= '2022-12-25')].drop_duplicates(subset=['Date','Name','Subject_Line','Segment','Variant'], keep='last').sort_values('Date')
    updated_masterlist = updated_masterlist.loc[(updated_masterlist['Date'] < '2022-12-25')].append(dedup)
    ...
    #print(updated_masterlist[['Name']].to_string())
    
    deduped = updated_masterlist.loc[(updated_masterlist['Date'] >= '2022-12-25')].drop_duplicates(subset=['Date','Name','Subject_Line','Segment','Variant'], keep='last').sort_values('Date')
    updated_masterlist = updated_masterlist.loc[(updated_masterlist['Date'] < '2022-12-25')].append(deduped)

    # Updating Old record by removing Duplicates
    missing_str_cols = ['Segment_Engagement','Segment_BNB','Segment_Freq','Error','Name_Bkp','Id','Top_Subject','Est_Opens']
    ...
    dbx_rename = {"Name": "Name","Campaign": "By Campaign","Mailing": "By Mailing","Test_Type": "Test Type","Variant": "Variant","Type_2": "Type 2","Offer": "Offer","Subject_Line": "Subject","Segment": "List","Date": "Date","ESP": "ESP","Type_0": "Type 0","Type_1": "Type 1","Sent": "Messages Sent","Delivered": "Delivered","Total_Bounces": "Hard Bounce","Unsub": "Opt Out","Opens": "Confirmed Opens","Clicks": "Clicks","Revenue": "Rev","Orders": "Purchases","GA_Revenue": "GA Revenue","GA_Orders": "GA Orders","GA_Sessions": "GA Sessions","Google_Analytics_Campaign_Name": "Google Analytics Campaign Name","Google_Analytics_Campaign_Content": "Google Analytics Campaign Content","Listrak_Conversion_Analytics_Campaign_Name": "Listrak Conversion Analytics Campaign Name","Listrak_Conversion_Analytics_Version": "Listrak Conversion Analytics Version","Listrak_Conversion_Analytics_Module_Name": "Listrak Conversion Analytics Module Name","Id": "Id","Top_Subject": "Top Subject","Est_Opens": "Est Opens","Soft_Bounces": "Soft Bounce","Complaints": "Spam Report"}
    dbx_schema = ["Name","By Campaign","By Mailing","Test Type","Variant","Type 2","Offer","Subject","List","Date","ESP","Type 0","Type 1","Messages Sent","Delivered","Hard Bounce","Opt Out","Confirmed Opens","Clicks","Rev","Purchases","GA Revenue","GA Orders","GA Sessions","Google Analytics Campaign Name","Google Analytics Campaign Content","Listrak Conversion Analytics Campaign Name","Listrak Conversion Analytics Version","Listrak Conversion Analytics Module Name","Id","Top Subject","Est Opens","Soft Bounce","Spam Report"]
...
def campaign_transform(campaign_data_df,client_id,client_esp,client_name):
    ...
    df['Variant'] = df['Variant'].str.replace(r"\(.*\)","")
    ...
    #df['Type_2'] = df['Type_2'].str.replace('-', "Other", regex=True)
...
def get_trigger_dates(file_name):
    ...
    temp_first_date = temp_file_name[-2]
